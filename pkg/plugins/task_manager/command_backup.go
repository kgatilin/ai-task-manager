...
func (c *BackupCommand) getDatabasePath(projectName string) string {
	return filepath.Join(c.Plugin.workingDir, ".darwinflow", "projects", projectName, "roadmap.db")
}
...
func (c *BackupCommand) getBackupDirectory(projectName string) string {
	...
	if err != nil {
		...
		return filepath.Join(c.Plugin.workingDir, ".darwinflow", "task-manager", "backups", projectName)
	}
