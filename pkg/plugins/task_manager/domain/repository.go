...
// GetLogger returns the logger for the given repository (if supported).
// This is a helper for backward compatibility with code that needs access to the logger.
func GetLogger(repo RoadmapRepository) pluginsdk.Logger {
	// Type assertion to access logger if available
	if r, ok := repo.(interface{ GetLogger() pluginsdk.Logger }); ok {
		return r.GetLogger()
	}
	return nil
}
