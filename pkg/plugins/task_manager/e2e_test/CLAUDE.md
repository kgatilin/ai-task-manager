# E2E Test Suite - Task Manager Plugin

## Overview

End-to-end tests validate the complete CLI command workflow by building the actual binary from source and executing real commands. Tests ensure CLI commands work correctly with database persistence, command parsing, and cross-entity operations.
...
**Set `DARWINFLOW_WORKING_DIR` for all commands:**
...
**Why**: Ensures consistent working directory for `.darwinflow/active-project.txt` and project databases.
...
- ❌ Write to real user directories (`~/.darwinflow`)
...
**Run specific suite:**
```bash
go test ./pkg/plugins/task_manager/e2e_test/... -v -run TestIterationSuite
```

**Run single test:**
```bash
go test ./pkg/plugins/task_manager/e2e_test/... -v -run TestIterationSuite/TestIterationCreate
```

**Expected behavior:**
- Binary builds once before all tests
...
**Last Updated**: 2025-11-14 (Iteration #34 - E2E Test Infrastructure)
