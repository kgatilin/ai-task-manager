...
import (
	...
	tm "github.com/kgatilin/darwinflow-pub/pkg/plugins/task_manager/presentation/cli"
	"github.com/kgatilin/darwinflow-pub/pkg/pluginsdk"
)

// MockLogger is a mock implementation of pluginsdk.Logger for testing
...
func (m *MockRepository) GetTask(ctx context.Context, id string) (*entities.TaskEntity, error) {
	return nil, pluginsdk.ErrNotFound
}
...
func (m *MockRepository) GetIterationTasks(ctx context.Context, iterationNum int) ([]*entities.TaskEntity, error) {
	if m.shouldError {
		return nil, pluginsdk.ErrInternal
	}
	// Return tasks that are in the iteration
	var iterationTasks []*entities.TaskEntity
	for _, iter := range m.iterations {
...
func (m *MockRepository) GetIterationByNumber(ctx context.Context, iterationNum int) (*entities.IterationEntity, error) {
	return m.GetIteration(ctx, iterationNum)
}
...
func (m *MockRepository) ListACByIteration(ctx context.Context, iterationNum int) ([]*entities.AcceptanceCriteriaEntity, error) {
	return []*entities.AcceptanceCriteriaEntity{}, nil
}
...
func TestAppModelInit(t *testing.T) {
	...
	repo := NewMockRepository()
	logger := NewMockLogger()
	repo.activeRoadmap = &entities.RoadmapEntity{
		ID:              "roadmap-1",
		Vision:          "Test vision",
...
func TestAppModelRoadmapListView(t *testing.T) {
	...
	model := tm.NewAppModel(ctx, repo, logger)
	model.SetRoadmap(&entities.RoadmapEntity{
		ID:              "roadmap-1",
		Vision:          "Test vision",
		SuccessCriteria: "Test criteria",
		CreatedAt:       time.Now(),
	...
	model.SetTracks([]*entities.TrackEntity{
		{
			ID:          "track-1",
			RoadmapID:   "roadmap-1",
			Title:       "Track 1",
...
func TestAppModelTrackDetailView(t *testing.T) {
	...
	model := tm.NewAppModel(ctx, repo, logger)
	model.SetCurrentTrack(&entities.TrackEntity{
		ID:          "track-1",
		RoadmapID:   "roadmap-1",
		Title:       "Track 1",
		Description: "Description 1",
	...
	model.SetTasks([]*entities.TaskEntity{
		{
			ID:          "task-1",
			TrackID:     "track-1",
			Title:       "Task 1",
...
func TestIterationListView(t *testing.T) {
	...
	now := time.Now()
	iter1, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{"task-1", "task-2"}, "complete", 500, now, now.AddDate(0, 0, 7), now, now)
	iter2, _ := entities.NewIterationEntity(2, "Sprint 2", "Features", "New features", []string{"task-3"}, "current", 500, now.AddDate(0, 0, 7), time.Time{}, now.AddDate(0, 0, 7), now.AddDate(0, 0, 7))

	repo.iterations = []*entities.IterationEntity{iter1, iter2}
...
func TestIterationListViewEmpty(t *testing.T) {
	...
	model := tm.NewAppModel(ctx, repo, logger)
	model.SetIterations([]*entities.IterationEntity{})
	model.SetCurrentView(tm.ViewIterationList)
...
func TestIterationDetailView(t *testing.T) {
	...
	// Create tasks
	task1, _ := entities.NewTaskEntity("task-1", "track-1", "Task 1", "Description 1", "done", 200, "", now, now)
	task2, _ := entities.NewTaskEntity("task-2", "track-1", "Task 2", "Description 2", "in-progress", 300, "", now, now)
	task3, _ := entities.NewTaskEntity("task-3", "track-1", "Task 3", "Description 3", "todo", 400, "", now, now)

	// Create iteration with tasks
	iter, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{"task-1", "task-2", "task-3"}, "current", 500, now, time.Time{}, now, now)

	repo.tasks = []*entities.TaskEntity{task1, task2, task3}

	model := tm.NewAppModel(ctx, repo, logger)
	model.SetCurrentIteration(iter)
	model.SetIterationTasks([]*entities.TaskEntity{task1, task2, task3})
...
func TestIterationDetailViewNoTasks(t *testing.T) {
	...
	now := time.Now()
	iter, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "planned", 500, time.Time{}, time.Time{}, now, now)

	model := tm.NewAppModel(ctx, repo, logger)
	model.SetCurrentIteration(iter)
	model.SetIterationTasks([]*entities.TaskEntity{})
	model.SetCurrentView(tm.ViewIterationDetail)
...
func TestNavigationToIterationList(t *testing.T) {
	...
	now := time.Now()
	iter1, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "complete", 500, now, now, now, now)
	repo.iterations = []*entities.IterationEntity{iter1}
	...
	model.SetTracks([]*entities.TrackEntity{})
	model.SetCurrentView(tm.ViewRoadmapList)
	model.SetIterations([]*entities.IterationEntity{iter1})
...
func TestNavigationFromIterationListToDetail(t *testing.T) {
	...
	now := time.Now()
	iter1, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "complete", 500, now, now, now, now)
	iter2, _ := entities.NewIterationEntity(2, "Sprint 2", "Features", "New features", []string{}, "current", 500, now, time.Time{}, now, now)
	repo.iterations = []*entities.IterationEntity{iter1, iter2}
...
func TestNavigationFromIterationDetailBack(t *testing.T) {
	...
	now := time.Now()
	iter, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "complete", 500, now, now, now, now)
...
func TestNavigationFromMainViewIterationSection(t *testing.T) {
	...
	iter1, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "complete", 500, now, now, now, now)
	iter2, _ := entities.NewIterationEntity(2, "Sprint 2", "Features", "New features", []string{}, "current", 500, now, time.Time{}, now, now)
	iter3, _ := entities.NewIterationEntity(3, "Sprint 3", "Polish", "Final touches", []string{}, "planned", 500, time.Time{}, time.Time{}, now, now)

	repo.iterations = []*entities.IterationEntity{iter1, iter2, iter3}
	...
	acs := []*entities.AcceptanceCriteriaEntity{
		{
			...
			VerificationType: entities.VerificationTypeManual,
			Status:           entities.ACStatusNotStarted,
			CreatedAt:        time.Now(),
			UpdatedAt:        time.Now(),
		},
		{
			...
			VerificationType: entities.VerificationTypeAutomated,
			Status:           entities.ACStatusPendingHumanReview,
			CreatedAt:        time.Now(),
			UpdatedAt:        time.Now(),
		},
	}
...
func TestACSpaceBarVerification(t *testing.T) {
	...
	if acs[0].Status != entities.ACStatusVerified {
	...
	acs := []*entities.AcceptanceCriteriaEntity{
		{
			...
			Description:      "Test AC 1",
			VerificationType: entities.VerificationTypeManual,
			Status:           entities.ACStatusVerified,
			CreatedAt:        time.Now(),
			UpdatedAt:        time.Now(),
		},
	}
...
func TestBacklogDisplayInMainView(t *testing.T) {
	...
	task1, _ := entities.NewTaskEntity("TM-task-1", "TM-track-1", "Backlog Task 1", "Description 1", "todo", 200, "", now, now)
	task2, _ := entities.NewTaskEntity("TM-task-2", "TM-track-1", "Backlog Task 2", "Description 2", "todo", 300, "", now, now)
	task3, _ := entities.NewTaskEntity("TM-task-3", "TM-track-1", "Done Task", "Should not appear", "done", 400, "", now, now)

	repo.activeRoadmap = roadmap
	repo.tracks = []*entities.TrackEntity{track1}
	repo.tasks = []*entities.TaskEntity{task1, task2, task3}
	...
	backlogTasks := []*entities.TaskEntity{task1, task2} // Exclude done task
...
func TestBacklogDisplayWithTrackInfo(t *testing.T) {
	...
	repo.activeRoadmap = roadmap
	repo.tracks = []*entities.TrackEntity{track1}
	repo.tasks = []*entities.TaskEntity{task1}
	...
	// Simulate loading backlog data
	backlogTasks := []*entities.TaskEntity{task1}
	msg := tm.FullRoadmapDataLoadedMsg{
		IterationTasks: make(map[int][]*entities.TaskEntity),
		TrackTasks:     make(map[string][]*entities.TaskEntity),
		BacklogTasks:   backlogTasks,
...
func TestBacklogTabNavigation(t *testing.T) {
	...
	// Create iteration
	iter1, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{}, "current", 500, now, time.Time{}, now, now)
...
func TestBacklogNavigationUpDown(t *testing.T) {
	...
	task1, _ := entities.NewTaskEntity("TM-task-1", "TM-track-1", "Backlog Task 1", "Description 1", "todo", 200, "", now, now)
	task2, _ := entities.NewTaskEntity("TM-task-2", "TM-track-1", "Backlog Task 2", "Description 2", "todo", 300, "", now, now)
	task3, _ := entities.NewTaskEntity("TM-task-3", "TM-track-1", "Backlog Task 3", "Description 3", "todo", 400, "", now, now)
	...
	// Simulate loading backlog data
	backlogTasks := []*entities.TaskEntity{task1, task2, task3}
	msg := tm.FullRoadmapDataLoadedMsg{
		IterationTasks: make(map[int][]*entities.TaskEntity),
		TrackTasks:     make(map[string][]*entities.TaskEntity),
		BacklogTasks:   backlogTasks,
...
func TestBacklogEmptyState(t *testing.T) {
	...
	msg := tm.FullRoadmapDataLoadedMsg{
		IterationTasks: make(map[int][]*entities.TaskEntity),
		TrackTasks:     make(map[string][]*entities.TaskEntity),
		BacklogTasks:   []*entities.TaskEntity{}, // Empty backlog
	}
	_, _ = model.Update(msg)
...
func TestBacklogSelectionHighlight(t *testing.T) {
	...
	repo.activeRoadmap = roadmap
	repo.tasks = []*entities.TaskEntity{task1, task2}
	...
	// Simulate loading backlog data
	backlogTasks := []*entities.TaskEntity{task1, task2}
	msg := tm.FullRoadmapDataLoadedMsg{
		IterationTasks: make(map[int][]*entities.TaskEntity),
		TrackTasks:     make(map[string][]*entities.TaskEntity),
		BacklogTasks:   backlogTasks,
...
func TestIterationDetailViewportScrollCommands(t *testing.T) {
	...
	now := time.Now()
	iter, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", []string{"task-1"}, "current", 500, now, time.Time{}, now, now)
	task1, _ := entities.NewTaskEntity("task-1", "track-1", "Task 1", "Description", "todo", 200, "", now, now)

	model := tm.NewAppModel(ctx, repo, logger)
	model.SetCurrentView(tm.ViewIterationDetail)
	model.SetCurrentIteration(iter)
	model.SetIterationTasks([]*entities.TaskEntity{task1})
	model.SetDimensions(80, 24)
...
func TestIterationDetailScrollVisibility(t *testing.T) {
	...
	tasks := make([]*entities.TaskEntity, 0)
	taskIDs := make([]string, 0)
	for i := 1; i <= 15; i++ {
		taskID := "task-" + string(rune('0'+i))
		task, _ := entities.NewTaskEntity(taskID, "track-1", "Task "+string(rune('0'+i)), "Description "+string(rune('0'+i)), "todo", 200+i*10, "", now, now)
		tasks = append(tasks, task)
		taskIDs = append(taskIDs, taskID)
	}

	// Create iteration with all tasks
	iter, _ := entities.NewIterationEntity(1, "Sprint 1", "Foundation", "Core features", taskIDs, "current", 500, now, time.Time{}, now, now)

	// Create many ACs for first few tasks to force scrolling
	acs := make([]*entities.AcceptanceCriteriaEntity, 0)
	for i := 0; i < 10; i++ {
...
func TestIterationDetailScrollVisibilityWithTestingInstructions(t *testing.T) {
	...
	task, _ := entities.NewTaskEntity("task-1", "track-1", "Task 1", "Description", "todo", 200, "", now, now)
	ac := entities.NewAcceptanceCriteriaEntity(
		"ac-1",
		"task-1",
	...
	repo.tasks = []*entities.TaskEntity{task}

	model := tm.NewAppModel(ctx, repo, logger)
	model.SetCurrentIteration(iter)
	model.SetIterationTasks([]*entities.TaskEntity{task})
	model.SetACs([]*entities.AcceptanceCriteriaEntity{ac})
	model.SetCurrentView(tm.ViewIterationDetail)
...
func TestTaskScrollVisibility(t *testing.T) {
	...
	for i := 1; i <= 30; i++ {
		...
		task, _ := entities.NewTaskEntity(taskID, "track-1", "Task "+string(rune('0'+i)), "Description", status, 200+i*10, "", now, now)
		tasks = append(tasks, task)
		taskIDs = append(taskIDs, taskID)
	}
