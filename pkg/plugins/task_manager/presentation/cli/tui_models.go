...
const (
	...
	ViewADRList
	ViewACList
	ViewACDetail    // View for AC detail (showing full description and testing instructions)
...
type AppModel struct {
	...
	currentTrack *entities.TrackEntity
	tasks       []*entities.TaskEntity
	currentTask *entities.TaskEntity

	// Data - Iteration views
	iterations       []*entities.IterationEntity
	currentIteration *entities.IterationEntity
	...
	selectedACIdx    int
	selectedACDetail *entities.AcceptanceCriteriaEntity // Selected AC for detail view

	// Input - for capturing AC failure feedback
	feedbackInput          textarea.Model
	selectedACForFeedback  *entities.AcceptanceCriteriaEntity // Stores the actual AC being marked as failed
	...
	// Full roadmap data
	iterationTasksByNumber map[int][]*entities.TaskEntity   // Tasks for each iteration
	trackTasksByID         map[string][]*entities.TaskEntity // Tasks for each track
	backlogTasks           []*entities.TaskEntity            // Tasks not in any iteration/track
...
func NewAppModel(
	...
	// Initialize textarea for AC failure feedback
	ta := textarea.New()
	ta.Placeholder = "Enter failure feedback..."
	ta.SetWidth(80)   // Will be updated on window resize
	ta.SetHeight(5)   // Multi-line height
	...
	m := &AppModel{
		...
		iterationTasksByNumber:  make(map[int][]*entities.TaskEntity),
		trackTasksByID:          make(map[string][]*entities.TaskEntity),
		backlogTasks:            make([]*entities.TaskEntity, 0),
		feedbackInput:           ta,
	}
...
func (m *AppModel) loadRoadmap() tea.Msg {
	...
	tracks, err := m.repository.ListTracks(m.ctx, roadmap.ID, entities.TrackFilters{})
	if err != nil {
		return RoadmapLoadedMsg{Error: err}
	}
...
func (m *AppModel) loadTrackDetail(trackID string) tea.Cmd {
	return func() tea.Msg {
		...
		tasks, err := m.repository.ListTasks(m.ctx, entities.TaskFilters{TrackID: trackID})
		if err != nil {
			return TrackDetailLoadedMsg{Error: err}
		}
...
func (m *AppModel) loadFullRoadmapData() tea.Cmd {
	return func() tea.Msg {
		// Load all tasks
		allTasks, err := m.repository.ListTasks(m.ctx, entities.TaskFilters{})
		if err != nil {
			return FullRoadmapDataLoadedMsg{Error: err}
		}

		// Create a map of tasks by ID for quick lookup
		tasksByID := make(map[string]*entities.TaskEntity)
		for _, task := range allTasks {
			tasksByID[task.ID] = task
		}

		// Initialize maps
		iterationTasks := make(map[int][]*entities.TaskEntity)
		trackTasks := make(map[string][]*entities.TaskEntity)
		assignedTaskIDs := make(map[string]bool) // Track which tasks are assigned to iterations
		var backlogTasks []*entities.TaskEntity
...
func (m *AppModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case ViewACDetail:
			return m.handleACDetailKeys(msg)
		case ViewACFailInput:
	...
	case tea.WindowSizeMsg:
		...
		// Update textarea width for AC failure feedback input
		m.feedbackInput.SetWidth(msg.Width - 2) // Leave 1 char margin on each side
		return m, nil
	...
	case RoadmapLoadedMsg:
		...
		if m.reorderingIterationNumber != 0 {
			...
			activeIterations := []*entities.IterationEntity{}
			for _, iter := range m.iterations {
...
func (m *AppModel) handleRoadmapListKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "j", "down":
		...
		case SelectTracks:
			// Navigate tracks
			activeTracks := []*entities.TrackEntity{}
...
func (m *AppModel) handleTrackDetailKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case " ": // Space bar - verify selected AC
		if m.selectedACIdx < len(m.acs) {
			...
			if ac.Status != entities.ACStatusVerified && ac.Status != entities.ACStatusAutomaticallyVerified {
...
func (m *AppModel) handleIterationDetailKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		if m.iterationDetailFocusAC {
			// View AC detail (when focused on ACs)
			...
			acIdx := 0
			var selectedAC *entities.AcceptanceCriteriaEntity
			...
			// Open AC detail view
			if selectedAC != nil {
				m.selectedACDetail = selectedAC
				m.previousViewMode = ViewIterationDetail
				m.currentView = ViewACDetail
			}
		} else if m.selectedIterationTaskIdx < len(m.iterationTasks) {
	...
	case " ": // Space bar
		if m.iterationDetailFocusAC {
			...
			if selectedAC != nil {
				if selectedAC.Status == entities.ACStatusVerified {
					// Unverify
					selectedAC.Status = entities.ACStatusNotStarted
				} else {
					// Verify
					selectedAC.Status = entities.ACStatusVerified
				}
...
func (m *AppModel) renderRoadmapList() string {
	...
	nonCompletedIterations := []*entities.IterationEntity{}
	completedIterations := []*entities.IterationEntity{}
	for _, iter := range m.iterations {
		if iter.Status == "complete" {
	...
	activeTracks := []*entities.TrackEntity{}
	completedTracks := []*entities.TrackEntity{}
	for _, track := range m.tracks {
		if track.Status == "complete" || track.Status == "done" {
...
func (m *AppModel) renderTrackDetail() string {
	...
	if adrs, err := m.repository.ListADRs(m.ctx, &m.currentTrack.ID); err == nil {
		if len(adrs) > 0 {
			...
			for _, adr := range adrs {
				if adr.Status == string(entities.ADRStatusAccepted) {
					acceptedCount++
				}
			}
	...
	if tasks, err := m.repository.ListTasks(m.ctx, entities.TaskFilters{TrackID: m.currentTrack.ID}); err == nil {
		totalACs := 0
		verifiedACs := 0
		pendingReviewACs := 0
		for _, task := range tasks {
			if acs, err := m.repository.ListAC(m.ctx, task.ID); err == nil {
				for _, ac := range acs {
					...
					if ac.Status == entities.ACStatusVerified || ac.Status == entities.ACStatusAutomaticallyVerified {
						verifiedACs++
					} else if ac.Status == entities.ACStatusPendingHumanReview {
						pendingReviewACs++
					}
				}
			}
...
func (m *AppModel) renderTaskDetail() string {
	...
	if acErr == nil && len(acs) > 0 {
		...
		for _, ac := range acs {
			...
			typeStr := "manual"
			if ac.VerificationType == entities.VerificationTypeAutomated {
				typeStr = "auto"
			}
...
// GetStatusIcon returns an icon representing the given status
...
func (m *AppModel) SetRoadmap(roadmap *entities.RoadmapEntity) {
	m.roadmap = roadmap
}

// SetTracks sets the tracks for testing
func (m *AppModel) SetTracks(tracks []*entities.TrackEntity) {
	m.tracks = tracks
}

// SetTasks sets the tasks for testing
func (m *AppModel) SetTasks(tasks []*entities.TaskEntity) {
	m.tasks = tasks
}

// SetCurrentTrack sets the current track for testing
func (m *AppModel) SetCurrentTrack(track *entities.TrackEntity) {
...
func (m *AppModel) SetIterations(iterations []*entities.IterationEntity) {
	m.iterations = iterations
}

// SetCurrentIteration sets the current iteration for testing
func (m *AppModel) SetCurrentIteration(iteration *entities.IterationEntity) {
	m.currentIteration = iteration
}

// SetIterationTasks sets the iteration tasks for testing
func (m *AppModel) SetIterationTasks(tasks []*entities.TaskEntity) {
...
func getADRStatusIcon(status string) string {
	switch status {
	case string(entities.ADRStatusAccepted):
		return "✓"
	case string(entities.ADRStatusProposed):
		return "○"
	case string(entities.ADRStatusDeprecated):
		return "✗"
	case string(entities.ADRStatusSuperseded):
		return "⇒"
	default:
		return "?"
	}
...
func (m *AppModel) renderACList() string {
	...
	if len(m.acs) == 0 {
		...
		for i, ac := range m.acs {
			...
			if ac.Status == entities.ACStatusFailed && ac.Notes != "" {
...
func (m *AppModel) renderACFailInput() string {
	...
	if m.selectedACForFeedback != nil {
		...
		// Help
		s += helpStyle.Render("Ctrl+D: Submit | esc: Cancel | ↑/↓: Navigate | Enter: New line")
	} else {
...
func (m *AppModel) handleACListKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		// View AC detail
		if m.selectedACIdx < len(m.acs) {
			m.selectedACDetail = m.acs[m.selectedACIdx]
...
func (m *AppModel) handleACFailInputKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	// Handle Ctrl+D for submission (common in terminal editors)
	if msg.Type == tea.KeyCtrlD {
		...
		if selectedAC != nil {
			...
			selectedAC.Status = entities.ACStatusFailed
	...
	// Update textarea with key press
...
func getACStatusIcon(status string) string {
	switch status {
	case string(entities.ACStatusVerified):
		return "✓"
	case string(entities.ACStatusAutomaticallyVerified):
		return "✓ₐ"
	case string(entities.ACStatusPendingHumanReview):
		return "⏸"
	case string(entities.ACStatusFailed):
		return "✗"
	case string(entities.ACStatusNotStarted):
		return "○"
	default:
		return "?"
	}
...
func (m *AppModel) SetACs(acs []*entities.AcceptanceCriteriaEntity) {
	m.acs = acs
}
...
// renderACDetail renders the AC detail view with full description and testing instructions
func (m *AppModel) renderACDetail() string {
	if m.selectedACDetail == nil {
		return "No AC detail to display"
	}

	ac := m.selectedACDetail
	var s string
	...
	titleStyle := lipgloss.NewStyle().
		...
		MarginBottom(1)
	statusIcon := getACStatusIcon(string(ac.Status))
	s += titleStyle.Render(fmt.Sprintf("AC: %s %s", statusIcon, ac.ID)) + "\n\n"

	statusStyle := lipgloss.NewStyle().
		Foreground(lipgloss.Color("243")).
		Italic(true)
	s += statusStyle.Render(fmt.Sprintf("Status: %s", ac.Status)) + "\n"
	s += statusStyle.Render(fmt.Sprintf("Task: %s", ac.TaskID)) + "\n"

	verifyType := "Manual"
	if ac.VerificationType == entities.VerificationTypeAutomated {
		verifyType = "Automated"
	}
	s += statusStyle.Render(fmt.Sprintf("Type: %s", verifyType)) + "\n"
	...
	if ac.TestingInstructions != "" {
		s += "\n" + sectionStyle.Render("Testing Instructions") + "\n"
		testWidth := m.width - 4
		if testWidth < 20 {
			testWidth = 20
		}
		renderedMarkdown := renderMarkdown(ac.TestingInstructions, testWidth)
		for _, line := range strings.Split(renderedMarkdown, "\n") {
			s += "  " + line + "\n"
		}
	}

	if ac.Status == entities.ACStatusFailed && ac.Notes != "" {
		s += "\n" + sectionStyle.Render("Failure Reason") + "\n"
		feedbackWidth := m.width - 4
		if feedbackWidth < 20 {
			feedbackWidth = 20
		}
		wrappedFeedback := WrapText(ac.Notes, feedbackWidth)
		for _, line := range strings.Split(wrappedFeedback, "\n") {
			s += "  " + line + "\n"
		}
	}
	...
	helpStyle := lipgloss.NewStyle().
		...
		Italic(true).
		MarginTop(1)
	s += "\n" + helpStyle.Render("esc: Back | space: Verify | f: Failed | q: Quit")
	return s
}

// handleACDetailKeys processes key presses in AC detail view
func (m *AppModel) handleACDetailKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	if m.selectedACDetail == nil {
		return m, nil
	}
	...
	case " ":
		if ac.Status == entities.ACStatusVerified {
			ac.Status = entities.ACStatusNotStarted
		} else if ac.Status != entities.ACStatusFailed {
			ac.Status = entities.ACStatusVerified
		}
		ac.UpdatedAt = time.Now()
		...
		if updated, err := m.repository.GetAC(m.ctx, ac.ID); err == nil {
			m.selectedACDetail = updated
		}

	case "f":
		if ac.Status != entities.ACStatusFailed {
			m.selectedACForFeedback = ac
			m.feedbackInput.SetValue("")
			m.feedbackInput.Focus()
			m.previousViewMode = ViewACDetail
			m.currentView = ViewACFailInput
		}
	}
...
// Test helper methods for iteration detail view

// SetIterationDetailFocusAC sets whether the iteration detail view is focused on ACs
func (m *AppModel) SetIterationDetailFocusAC(focusAC bool) {
	m.iterationDetailFocusAC = focusAC
}

// GetSelectedIterationACIdx returns the currently selected AC index in iteration view
func (m *AppModel) GetSelectedIterationACIdx() int {
	return m.selectedIterationACIdx
}

// GetSelectedIterationTaskIdx returns the currently selected task index in iteration view
func (m *AppModel) GetSelectedIterationTaskIdx() int {
	return m.selectedIterationTaskIdx
}

// Stub methods for tests - scroll line tracking not yet implemented

// GetACStartLines returns empty map (feature not yet implemented)
func (m *AppModel) GetACStartLines() map[string]int {
	return make(map[string]int)
}

// GetTaskStartLines returns empty map (feature not yet implemented)
func (m *AppModel) GetTaskStartLines() map[string]int {
	return make(map[string]int)
}

// GetIterationDetailViewportYOffset returns 0 (feature not yet implemented)
func (m *AppModel) GetIterationDetailViewportYOffset() int {
	return 0
}
