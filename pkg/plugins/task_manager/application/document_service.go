...
func (s *DocumentApplicationService) CreateDocument(ctx context.Context, input dto.CreateDocumentDTO) (string, error) {
	// Validate title
	if input.Title == "" {
		return "", fmt.Errorf("%w: document title is required", pluginsdk.ErrInvalidArgument)
	}
	if len(input.Title) > 200 {
		return "", fmt.Errorf("%w: document title must be 200 characters or less", pluginsdk.ErrInvalidArgument)
	}
	...
	// Validate content
	if input.Content == "" {
		return "", fmt.Errorf("%w: document content is required", pluginsdk.ErrInvalidArgument)
	}

	// Validate XOR for TrackID and IterationNumber
	if input.TrackID != nil && input.IterationNumber != nil {
		return "", fmt.Errorf("%w: document cannot have both TrackID and IterationNumber (choose one or neither)", pluginsdk.ErrInvalidArgument)
	}

	// Validate TrackID format if provided
	if input.TrackID != nil && *input.TrackID != "" {
		if !isValidTrackIDFormat(*input.TrackID) {
			return "", fmt.Errorf("%w: invalid track ID format: %s", pluginsdk.ErrInvalidArgument, *input.TrackID)
		}
		// Verify track exists
