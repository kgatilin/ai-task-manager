...
func (c *ProjectCreateCommand) Execute(ctx context.Context, cmdCtx pluginsdk.CommandContext, args []string) error {
	...
	projectDir := filepath.Join(c.Provider.GetWorkingDir(), ".darwinflow", "projects", c.projectName)
	...
	// The GetProjectDatabase call ensures the database and tables are created
	if _, err := c.Provider.GetProjectDatabase(c.projectName); err != nil {
		return fmt.Errorf("failed to initialize project database: %w", err)
	}

	// Get repository for the newly created project
	repo, cleanup, err := c.Provider.GetRepositoryForProject(c.projectName)
	if err != nil {
...
func (c *ProjectListCommand) Execute(ctx context.Context, cmdCtx pluginsdk.CommandContext, args []string) error {
	...
	projectsDir := filepath.Join(c.Provider.GetWorkingDir(), ".darwinflow", "projects")
	...
	// Get active project
	activeProject, err := c.Provider.GetActiveProject()
	if err != nil {
...
func (c *ProjectSwitchCommand) Execute(ctx context.Context, cmdCtx pluginsdk.CommandContext, args []string) error {
	...
	if err := c.Provider.SetActiveProject(c.projectName); err != nil {
