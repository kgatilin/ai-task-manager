...
func NewSQLiteRepositoryComposite(db *sql.DB, logger logger.Logger) *SQLiteRepositoryComposite {
...
func (c *SQLiteRepositoryComposite) Close() error {
	if c.DB != nil {
		return c.DB.Close()
	}
	return nil
...
func NewSQLiteRoadmapRepository(db *sql.DB, logger logger.Logger) domain.RoadmapRepository {
