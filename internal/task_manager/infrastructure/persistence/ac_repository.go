...
func NewSQLiteAcceptanceCriteriaRepository(db *sql.DB, logger logger.Logger) *SQLiteAcceptanceCriteriaRepository {
...
func (r *SQLiteAcceptanceCriteriaRepository) SaveAC(ctx context.Context, ac *entities.AcceptanceCriteriaEntity) error {
	...
	if exists > 0 {
		return fmt.Errorf("%w: AC %s already exists", tmerrors.ErrAlreadyExists, ac.ID)
	}
	...
	if taskExists == 0 {
		return fmt.Errorf("%w: task %s not found", tmerrors.ErrNotFound, ac.TaskID)
	}
...
func (r *SQLiteAcceptanceCriteriaRepository) GetAC(ctx context.Context, id string) (*entities.AcceptanceCriteriaEntity, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("%w: AC %s not found", tmerrors.ErrNotFound, id)
		}
...
func (r *SQLiteAcceptanceCriteriaRepository) UpdateAC(ctx context.Context, ac *entities.AcceptanceCriteriaEntity) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("%w: AC %s not found", tmerrors.ErrNotFound, ac.ID)
	}
...
func (r *SQLiteAcceptanceCriteriaRepository) DeleteAC(ctx context.Context, id string) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("%w: AC %s not found", tmerrors.ErrNotFound, id)
	}
