...
func NewSQLiteIterationRepository(db *sql.DB, logger logger.Logger, acRepo repositories.AcceptanceCriteriaRepository) *SQLiteIterationRepository {
...
func (r *SQLiteIterationRepository) SaveIteration(ctx context.Context, iteration *entities.IterationEntity) error {
	...
	if exists > 0 {
		return fmt.Errorf("%w: iteration %d already exists", tmerrors.ErrAlreadyExists, iteration.Number)
	}
...
func (r *SQLiteIterationRepository) GetIteration(ctx context.Context, number int) (*entities.IterationEntity, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, number)
		}
...
func (r *SQLiteIterationRepository) GetCurrentIteration(ctx context.Context) (*entities.IterationEntity, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("%w: no current iteration found", tmerrors.ErrNotFound)
		}
...
func (r *SQLiteIterationRepository) UpdateIteration(ctx context.Context, iteration *entities.IterationEntity) error {
	...
	if rows == 0 {
		return fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, iteration.Number)
	}
...
func (r *SQLiteIterationRepository) DeleteIteration(ctx context.Context, number int) error {
	...
	if rows == 0 {
		return fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, number)
	}
...
func (r *SQLiteIterationRepository) AddTaskToIteration(ctx context.Context, iterationNum int, taskID string) error {
	...
	if iterExists == 0 {
		return fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, iterationNum)
	}
	...
	if taskExists == 0 {
		return fmt.Errorf("%w: task %s not found", tmerrors.ErrNotFound, taskID)
	}
	...
	if alreadyExists > 0 {
		return fmt.Errorf("%w: task already in iteration", tmerrors.ErrAlreadyExists)
	}
...
func (r *SQLiteIterationRepository) RemoveTaskFromIteration(ctx context.Context, iterationNum int, taskID string) error {
	...
	if rows == 0 {
		return fmt.Errorf("%w: task not in iteration", tmerrors.ErrNotFound)
	}
...
func (r *SQLiteIterationRepository) GetIterationTasks(ctx context.Context, iterationNum int) ([]*entities.TaskEntity, error) {
	...
	if exists == 0 {
		return nil, fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, iterationNum)
	}
...
func (r *SQLiteIterationRepository) GetIterationTasksWithWarnings(ctx context.Context, iterationNum int) ([]*entities.TaskEntity, []string, error) {
	...
	if exists == 0 {
		return nil, nil, fmt.Errorf("%w: iteration %d not found", tmerrors.ErrNotFound, iterationNum)
	}
...
func (r *SQLiteIterationRepository) CompleteIteration(ctx context.Context, iterationNum int) error {
	...
	if iteration.Status != "current" {
		return fmt.Errorf("%w: iteration must be in current status to complete", tmerrors.ErrInvalidArgument)
	}
...
func (r *SQLiteIterationRepository) GetNextPlannedIteration(ctx context.Context) (*entities.IterationEntity, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("%w: no planned iteration found", tmerrors.ErrNotFound)
		}
