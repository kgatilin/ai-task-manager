...
func (s *ValidationService) ValidateTrackID(id string) error {
	...
	return fmt.Errorf("%w: track ID must follow convention: track-<slug> or <CODE>-track-<number>", errors.ErrInvalidArgument)
}
...
func (s *ValidationService) ValidateIterationNumber(number int) error {
	if number <= 0 {
		return fmt.Errorf("%w: iteration number must be positive", errors.ErrInvalidArgument)
	}
	return nil
}
...
func (s *ValidationService) ValidateRank(rank int) error {
	if rank < 1 || rank > 1000 {
		return fmt.Errorf("%w: rank must be between 1 and 1000", errors.ErrInvalidArgument)
	}
	return nil
}
...
func (s *ValidationService) ValidateNonEmpty(fieldName, value string) error {
	if value == "" {
		return fmt.Errorf("%w: %s must be non-empty", errors.ErrInvalidArgument, fieldName)
	}
	return nil
}
