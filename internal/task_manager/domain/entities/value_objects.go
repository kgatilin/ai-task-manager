...
func NewDocumentType(docType string) (DocumentType, error) {
	if !validDocumentTypes[docType] {
		return "", fmt.Errorf("%w: invalid document type: must be one of adr, plan, retrospective, other", errors.ErrInvalidArgument)
	}
...
func NewDocumentStatus(status string) (DocumentStatus, error) {
	if !validDocumentStatuses[status] {
		return "", fmt.Errorf("%w: invalid document status: must be one of draft, published, archived", errors.ErrInvalidArgument)
	}
