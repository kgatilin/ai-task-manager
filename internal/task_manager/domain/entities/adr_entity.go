...
func NewADREntity(
	...
	// Validate status
	if !IsValidADRStatus(status) {
		return nil, fmt.Errorf("%w: invalid ADR status: must be one of proposed, accepted, deprecated, superseded", errors.ErrInvalidArgument)
	}

	// Validate required fields
	if title == "" {
		return nil, fmt.Errorf("%w: ADR title is required", errors.ErrInvalidArgument)
	}
	if context == "" {
		return nil, fmt.Errorf("%w: ADR context is required", errors.ErrInvalidArgument)
	}
	if decision == "" {
		return nil, fmt.Errorf("%w: ADR decision is required", errors.ErrInvalidArgument)
	}
	if consequences == "" {
		return nil, fmt.Errorf("%w: ADR consequences are required", errors.ErrInvalidArgument)
	}

	// If superseded, SupersededBy must be set
	if status == string(ADRStatusSuperseded) && supersededBy == nil {
		return nil, fmt.Errorf("%w: superseded ADR must specify superseded_by", errors.ErrInvalidArgument)
	}
