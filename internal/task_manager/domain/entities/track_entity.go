...
func NewTrackEntity(id, roadmapID, title, description, status string, rank int, dependencies []string, createdAt, updatedAt time.Time) (*TrackEntity, error) {
	...
	if !isValidTrackID(id) {
		return nil, fmt.Errorf("%w: track ID must follow convention: track-<slug>", errors.ErrInvalidArgument)
	}

	// Validate status
	if !IsValidTrackStatus(status) {
		return nil, fmt.Errorf("%w: invalid track status: must be one of not-started, in-progress, complete, blocked, waiting", errors.ErrInvalidArgument)
	}

	// Validate rank
	if rank < 1 || rank > 1000 {
		return nil, fmt.Errorf("%w: invalid track rank: must be between 1 and 1000", errors.ErrInvalidArgument)
	}

	// Check for self-dependency
	for _, dep := range dependencies {
		if dep == id {
			return nil, fmt.Errorf("%w: track cannot depend on itself", errors.ErrInvalidArgument)
		}
	}
...
// Enforces state machine rules: complete is terminal
func (t *TrackEntity) TransitionTo(newStatus string) error {
	// Validate new status is valid
	if !IsValidTrackStatus(newStatus) {
		return fmt.Errorf("%w: invalid track status: %s", errors.ErrInvalidArgument, newStatus)
	}
	...
	if t.Status == string(TrackStatusComplete) && newStatus != string(TrackStatusComplete) {
		return fmt.Errorf("%w: cannot transition from complete to %s (complete is terminal)", errors.ErrInvalidArgument, newStatus)
	}
...
func (t *TrackEntity) GetProgress() float64 {
	switch t.Status {
	case string(TrackStatusComplete):
		return 1.0
	case string(TrackStatusInProgress):
		return 0.5
...
func (t *TrackEntity) IsBlocked() bool {
	return t.Status == string(TrackStatusBlocked)
}
...
func (t *TrackEntity) AddDependency(trackID string) error {
	// Check for self-dependency
	if trackID == t.ID {
		return fmt.Errorf("%w: track cannot depend on itself", errors.ErrInvalidArgument)
	}

	// Check if dependency already exists
	for _, dep := range t.Dependencies {
		if dep == trackID {
			return fmt.Errorf("%w: track %s already depends on %s", errors.ErrAlreadyExists, t.ID, trackID)
		}
	}
...
func (t *TrackEntity) RemoveDependency(trackID string) error {
	...
	if !found {
		return fmt.Errorf("%w: track %s does not depend on %s", errors.ErrNotFound, t.ID, trackID)
	}
