...
func NewDocumentEntity(
	...
	if !isValidDocumentID(id) {
		return nil, fmt.Errorf("%w: document ID must follow convention: TM-doc-<random>", errors.ErrInvalidArgument)
	}

	// Validate title
	if title == "" {
		return nil, fmt.Errorf("%w: document title is required", errors.ErrInvalidArgument)
	}
	if len(title) > 200 {
		return nil, fmt.Errorf("%w: document title must be 200 characters or less", errors.ErrInvalidArgument)
	}

	// Validate document type
	if !docType.IsValid() {
		return nil, fmt.Errorf("%w: invalid document type: %s", errors.ErrInvalidArgument, docType)
	}

	// Validate document status
	if !status.IsValid() {
		return nil, fmt.Errorf("%w: invalid document status: %s", errors.ErrInvalidArgument, status)
	}

	// Validate attachment constraints: XOR (either TrackID or IterationNumber, not both)
	if trackID != nil && iterationNumber != nil {
		return nil, fmt.Errorf("%w: document cannot have both TrackID and IterationNumber (choose one or neither)", errors.ErrInvalidArgument)
	}

	// Validate TrackID format if provided
	if trackID != nil && *trackID != "" {
		if !isValidTrackID(*trackID) {
			return nil, fmt.Errorf("%w: invalid track ID format: %s", errors.ErrInvalidArgument, *trackID)
		}
	}

	// Validate IterationNumber if provided
	if iterationNumber != nil && *iterationNumber < 1 {
		return nil, fmt.Errorf("%w: iteration number must be >= 1", errors.ErrInvalidArgument)
	}
...
func (d *DocumentEntity) UpdateStatus(status DocumentStatus) error {
	if !status.IsValid() {
		return fmt.Errorf("%w: invalid document status: %s", errors.ErrInvalidArgument, status)
	}
	d.Status = status
	d.UpdatedAt = time.Now()
	return nil
...
func (d *DocumentEntity) AttachToTrack(trackID string) error {
	if !isValidTrackID(trackID) {
		return fmt.Errorf("%w: invalid track ID format: %s", errors.ErrInvalidArgument, trackID)
	}
	if d.IterationNumber != nil && *d.IterationNumber > 0 {
		return fmt.Errorf("%w: document is already attached to an iteration", errors.ErrInvalidArgument)
	}
	d.TrackID = &trackID
...
func (d *DocumentEntity) AttachToIteration(iterationNumber int) error {
	if iterationNumber < 1 {
		return fmt.Errorf("%w: iteration number must be >= 1", errors.ErrInvalidArgument)
	}
	if d.TrackID != nil && *d.TrackID != "" {
		return fmt.Errorf("%w: document is already attached to a track", errors.ErrInvalidArgument)
	}
