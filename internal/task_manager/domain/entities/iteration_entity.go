...
func NewIterationEntity(number int, name, goal, deliverable string, taskIDs []string, status string, rank float64, startedAt, completedAt, createdAt, updatedAt time.Time) (*IterationEntity, error) {
	...
	if number <= 0 {
		return nil, fmt.Errorf("%w: iteration number must be positive", errors.ErrInvalidArgument)
	}

	// Validate status
	if !IsValidIterationStatus(status) {
		return nil, fmt.Errorf("%w: invalid iteration status: must be one of planned, current, complete", errors.ErrInvalidArgument)
	}

	// Validate rank (allow fractional values for fine-grained ordering)
	if rank < 1 || rank > 1000 {
		return nil, fmt.Errorf("%w: invalid iteration rank: must be between 1 and 1000", errors.ErrInvalidArgument)
	}
...
// - planned -> current (via Start)
// - current -> complete (via Complete)
func (i *IterationEntity) TransitionTo(newStatus string) error {
	// Validate new status is valid
	if !IsValidIterationStatus(newStatus) {
		return fmt.Errorf("%w: invalid iteration status: %s", errors.ErrInvalidArgument, newStatus)
	}

	// Enforce state transitions
	switch i.Status {
	case string(IterationStatusPlanned):
		if newStatus != string(IterationStatusCurrent) && newStatus != string(IterationStatusPlanned) {
			return fmt.Errorf("%w: can only transition from planned to current", errors.ErrInvalidArgument)
		}
	case string(IterationStatusCurrent):
		if newStatus != string(IterationStatusComplete) && newStatus != string(IterationStatusCurrent) {
			return fmt.Errorf("%w: can only transition from current to complete", errors.ErrInvalidArgument)
		}
	case string(IterationStatusComplete):
		if newStatus != string(IterationStatusComplete) {
			return fmt.Errorf("%w: cannot transition from complete (terminal state)", errors.ErrInvalidArgument)
		}
	}
	...
	// Set timestamps
	if newStatus == string(IterationStatusCurrent) && i.StartedAt == nil {
		now := time.Now()
		i.StartedAt = &now
	}
	if newStatus == string(IterationStatusComplete) && i.CompletedAt == nil {
		now := time.Now()
		i.CompletedAt = &now
	}
...
func (i *IterationEntity) Revert() error {
	if i.Status != string(IterationStatusComplete) {
		return fmt.Errorf("%w: can only revert completed iterations", errors.ErrInvalidArgument)
	}
...
func (i *IterationEntity) AddTask(taskID string) error {
	...
	for _, id := range i.TaskIDs {
		if id == taskID {
			return fmt.Errorf("%w: task already in iteration", errors.ErrAlreadyExists)
		}
	}
...
func (i *IterationEntity) RemoveTask(taskID string) error {
	...
	if !found {
		return fmt.Errorf("%w: task not in iteration", errors.ErrNotFound)
	}
