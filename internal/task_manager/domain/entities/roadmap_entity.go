...
func NewRoadmapEntity(id, vision, successCriteria string, createdAt, updatedAt time.Time) (*RoadmapEntity, error) {
	if vision == "" {
		return nil, fmt.Errorf("%w: vision must be non-empty", errors.ErrInvalidArgument)
	}
	if successCriteria == "" {
		return nil, fmt.Errorf("%w: success criteria must be non-empty", errors.ErrInvalidArgument)
	}
